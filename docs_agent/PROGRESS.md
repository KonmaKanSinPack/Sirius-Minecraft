...
## 当前阶段：M4（反射层）**已完成**——等级框架（L0/L1/L2 预留）+ ReflexScheduler 调度器 + L1 七条脊髓反射 + 事后知会，真机多项 PASS（零 VLM token）；pytest 339 / 冒烟 353 全绿。**等用户反馈决定下一轮**（候选：M5 分层大脑 / L2 战斗 / 多人补验）
...
- **L2（guard）战斗模块**：枚举位+instincts 预留节已就位；实现后置独立轮次（客户端战斗是最难件；接入需 ①战斗反射链 ②同步修订"禁止攻击"安全约束 ③切换放行 ④消费 entities health 字段）
- **多人补验**：等级切换聊天命令端到端（单机无第二玩家，bot 自回显过滤——命令解析单测覆盖、切换执行路径真机已验）+ speaking_look 有对象
- **着火/脱困真机补测**：需火源（开作弊测试世界/岩浆/打火石）与可复现卡位点；逻辑单测覆盖，触发通道是 M2-B 已验的 CRITICAL fire 事件
- **调度分层候选**：health_low/death 的"上报"与撤离的"行走"同层串行（单候选）——追击场景低血上报被撤离行走阻塞 ~10s
